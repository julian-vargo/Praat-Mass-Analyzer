# COMMAND LINE USERS: CONFIGURE YOUR VARIABLES HERE
    ...
    i_want_formants = 0
    i_want_pitch=0
    i_want_harmonicity=0
    i_want_intensity=0
    i_want_jitter_and_shimmer=0
    i_want_fricative_measurements=0
    i_want_A1P0=0
    ...
    # Leave blank to analyze all phonemes
    phonemes_to_be_analyzed$ = ""
...
form: "Vargo 2025 - Praat Mass Analyzer User Interface"
    ...
    comment: ""
    comment: "-----"
    comment: "Advanced settings: Do not touch. This section is reserved for command line users"
...
if developer_mode = 0
    beginPause: "Core Arguments"
        ...
        comment: "If you'd like details from a third or fourth tier, which number is it? Leave as 0 if none."
    ...
    beginPause: "Phonemes to Analyze"
        comment: "Do you want to clean-up or remove accidental tab characters,"
        comment: "newline character, or space characters from your phone tier?"
        boolean: "I want to clean up phone tier", 1
        comment: ""
        comment: "Now list all phonemes on the phone tier that you want to analyze"
        comment: " - Eg. To measure vowels and engma, type 'aeiouŋ'"
        comment: " - Case sensitive"
        comment: " - Leave blank to analyze all phonemes in your TextGrids"
        sentence: "Phonemes to be analyzed", ""
    clicked = endPause: "Submit and Continue to Next Page", 1

    beginPause: "Advanced settings"
        positive: "Pitch floor", 50
        positive: "Pitch ceiling", 500
...
if developer_mode = 0
    if i_want_voice_analysis = 1
        beginPause: "Voice analysis settings"
            comment: "Integer value: maximum pitch candidates"
            integer: "Maximum_candidates", 15
            boolean: "cc accuracy", 0
...
# If you get an error at the line below, your Praat version is out of date.
# If you have an older version of Praat, then the clock() function won't work. In that case, remove this whole line and remove the clock
# timer variables at the very end of the script. I use these timers to test that the script runs efficiently, but they're not that important.
runtimer = clock()
...
for file to numberOfFiles
    if ((file - 1) mod number_of_cpu_cores) = (process_index - 1)
        ...
        if i_want_voice_analysis =1
            select Sound 'currentSound$'
            # The arguments for To Pitch (raw cc)... are hard to find
            # Info is on line 507 of https://github.com/praat/praat.github.io/blob/master/fon/Sound_to_Pitch.cpp
            # They are timeStep, pitchFloor, pitchCeiling, maxnCandidates, veryAccurate, silenceThreshold, voicingThreshold, octaveCost, octaveJumpCost, voicedUnvoicedCost            
            currentPitchCC = noprogress To Pitch (raw cross-correlation)... 0 pitch_floor pitch_ceiling maximum_candidates cc_accuracy silence_threshold voicing_threshold octave_cost octave_jump_cost voiced_unvoiced_cost

            # This might be redundant but I'm not entirely sure how explicit I need to be about object selection
            select Sound 'currentSound$'
            plusObject: currentPitchCC
            currentPulses = To PointProcess (cc)
        endif
        ...
        for currentInterval from 1 to numberOfIntervals
            ...
            if i_want_to_clean_up_phone_tier = 1
                thisPhoneme$ = replace$(thisPhoneme$, tab$, "", 0)
                thisPhoneme$ = replace$(thisPhoneme$, " ", "", 0)
                thisPhoneme$ = replace$(thisPhoneme$, newline$, "", 0)
            endif
            if not thisPhoneme$ = ""
                if not thisPhoneme$ = "sil"
                    if not thisPhoneme$ = "<unk>"
                        analyze_this_phoneme_boolean = 0
                        if index(phonemes_to_be_analyzed$, thisPhoneme$) <> 0
                            analyze_this_phoneme_boolean = 1
                        endif
                        if phonemes_to_be_analyzed$ = ""
                            analyze_this_phoneme_boolean = 1
                        endif
                        if analyze_this_phoneme_boolean = 1
                            ...
                            if i_want_jitter_and_shimmer = 1
                                for t from 1 to 9
                                    ...
                                    jitter[t] = Get jitter (local, absolute)... timePercentile[t-1] timePercentile[t+1] period_floor period_ceiling maximum_period_factor
                                    selectObject: currentSound, currentPointProcess
                                    shimmer[t] = Get shimmer (local): timePercentile[t-1], timePercentile[t+1], period_floor, period_ceiling, maximum_period_factor, maximum_amplitude_factor
                            ...
                            # CPP, MFCC, smoothing, looking into cepstrum tables like how PraatSauce does
                            # Subharmonic to harmonic ratio
                            # Strength of Excitation
                            # Number of peaks below a certain Hz cutoff, which Tarun Pruthi and Carol Espy Wilson used to use in their 2004 and 2007 nasalization papers
                            if i_want_advanced_parameters = 1
                                selectObject: currentSound
                                for t from 1 to 10
                                    ...
                                    # The results file won't print any 99999 values
                                    ltas_f0[t] = 99999
                                    ltas_f1[t] = 99999
                                    ltas_f2[t] = 99999
                                    ltas_f3[t] = 99999
                            ...
                            if i_want_voice_analysis = 1
                                selectObject: currentPulses
                                n_pulses_high = Get high index... thisPhonemeEndTime
                                n_pulses_low = Get low index... thisPhonemeStartTime
                                n_pulses_interval = n_pulses_high - n_pulses_low

                                select Sound 'currentSound$'
                                plusObject: currentPitchCC
                                plusObject: currentPulses
                                
                                voiceReport$ = "99999"
                                if n_pulses_interval > 2
                                    # start, end, pitchfloor, pitchceiling, maxPeriodFactor, maxAmplitudeFactor, silenceThreshold, voicingThreshold
                                    # The four last value defaults in Praat are 1.3, 1.6, 0.03, 0.45
                                    voiceReport$ = nocheck Voice report: thisPhonemeStartTime, thisPhonemeEndTime, pitch_floor, pitch_ceiling, maximum_period_factor, maximum_amplitude_factor, silence_threshold, voicing_threshold
                                    # appendInfoLine: newline$, voiceReport$
                                endif

                                # The voice report is just one giant string variable, so we need to decompose it
                                voice_pitch_median$ = fixed$ (extractNumber (voiceReport$, "Median pitch:"), 4)
                                voice_pitch_mean$ = fixed$ (extractNumber (voiceReport$, "Mean pitch:"), 4)
                                voice_pitch_sd$ = fixed$ (extractNumber (voiceReport$, "Standard deviation:"), 4)
                                voice_pitch_min$ = fixed$ (extractNumber (voiceReport$, "Minimum pitch:"), 4)
                                voice_pitch_max$ = fixed$ (extractNumber (voiceReport$, "Maximum pitch:"), 4)
                                voice_pulses_n$ = fixed$ (extractNumber (voiceReport$, "Number of pulses:"), 4)
                                voice_periods_n$ = fixed$ (extractNumber (voiceReport$, "Number of periods:"), 4)
                                voice_period_mean$ = fixed$ (extractNumber (voiceReport$, "Mean period:"), 4)
                                voice_period_sd$ = fixed$ (extractNumber (voiceReport$, "Standard deviation of period:"), 4)
                                voice_unvoiced_fraction$ = fixed$ (extractNumber (voiceReport$, "Fraction of locally unvoiced frames:"), 4)
                                voice_voicebreaks_n$ = fixed$ (extractNumber (voiceReport$, "Number of voice breaks:"), 4)
                                voice_voicebreaks_degree$ = fixed$ (extractNumber (voiceReport$, "Degree of voice breaks:"), 4)
                                voice_jitter_local$ = fixed$ (extractNumber (voiceReport$, "Jitter (local):"), 4)
                                voice_jitter_local_abs$ = fixed$ (extractNumber (voiceReport$, "Jitter (local, absolute):"), 4)
                                voice_jitter_rap$ = fixed$ (extractNumber (voiceReport$, "Jitter (rap):"), 4)
                                voice_jitter_ppq5$ = fixed$ (extractNumber (voiceReport$, "Jitter (ppq5):"), 4)
                                voice_jitter_ddp$ = fixed$ (extractNumber (voiceReport$, "Jitter (ddp):"), 4)
                                voice_shimmer_local$ = fixed$ (extractNumber (voiceReport$, "Shimmer (local):"), 4)
                                voice_shimmer_local_db$ = fixed$ (extractNumber (voiceReport$, "Shimmer (local, dB):"), 4)
                                voice_shimmer_apq3$ = fixed$ (extractNumber (voiceReport$, "Shimmer (apq3):"), 4)
                                voice_shimmer_apq5$ = fixed$ (extractNumber (voiceReport$, "Shimmer (apq5):"), 4)
                                voice_shimmer_apq11$ = fixed$ (extractNumber (voiceReport$, "Shimmer (apq11):"), 4)
                                voice_shimmer_dda$ = fixed$ (extractNumber (voiceReport$, "Shimmer (dda):"), 4)
                                voice_autocorr_mean$ = fixed$ (extractNumber (voiceReport$, "Mean autocorrelation:"), 4)
                                voice_nhr_mean$ = fixed$ (extractNumber (voiceReport$, "Mean noise-to-harmonics ratio:"), 4)
                                voice_hnr_mean$ = fixed$ (extractNumber (voiceReport$, "Mean harmonics-to-noise ratio:"), 4)
                                voice_h1h2_points$ = fixed$ (extractNumber (voiceReport$, "Mean of the points:"), 4)
                                voice_h1h2_curve$ = fixed$ (extractNumber (voiceReport$, "Mean of the curve:"), 4)
            ...
            # This chunk removes any undefined or 99999 bad values, replacing those parts with empty strings
            results$ = replace$(results$, "--undefined--", "", 0)
            results$ = replace$(results$, "-99999", "", 0)
            results$ = replace$(results$, "99999", "", 0)

            # We append a single row of results to the end of our csv file, slowly building it up line by line
            appendFile: csv_file_path$, results$

            # Clear the results variable to keep memory usage low before starting a new phone of analysis.
            results$ = ""
        endfor

        # If this section errors for you, I suggest updating Praat to a newer version
        # Or, in the future, I plan on adding for loops here to check if I create the object before attempting its removal
        # I use the nocheck because it's convenient.
...
# If you got errors way earlier in the script about there being no existing function clock()
# Then you'll need to delete this entire section
# This code's not that important. I just use it to troubleshoot and make sure that the script is running quickly, so you can delete it.
timeElapsed = 0
